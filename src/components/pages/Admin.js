...
class Admin extends Component {
	...
	deleteRoom = (id, roomId) => {
		...
		if (window.confirm("Are you sure you want to delete this?")) {
			this.props.deleteRoom(id, roomId).then(window.location.reload());
		}
	};
	render() {
							...
							return (
								<Card key={company._id}>
									...
									<Accordion.Collapse eventKey={company._id}>
										<Card.Body>
											<Link to={`/addImg/${company._id}`}>
												<img
													src={company.imageURL}
													className="my-2 rounded-circle w-25 ml-auto "
													alt={company._id}
												/>
											</Link>
											<div className="d-flex justify-content-between">
												<Link to={`/addRoom/${company._id}`}>
													<p className="p-1">Add Room</p>
												</Link>{" "}
												<Link to={`/editCompany/${company._id}`}>
													<p className="p-1">Edit Company</p>
												</Link>
															...
															<Fragment key={room._id}>
																<tr style={{ textAlign: "center" }}>
																	...
																	<td>
																		{" "}
																		<Link
																			to={`/editRoom/${company._id}/${room._id}`}
																		>
																			<Button
																				variant="warning"
																				className="mr-2"
																		...
																		<Button
																			variant="danger"
																			onClick={() => {
																				this.deleteRoom(company._id, room._id);
																			}}
																		>
																			Delete
																		</Button>
